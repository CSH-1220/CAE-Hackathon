...
class CTranModel(nn.Module):
    ...
    def forward(self,images,mask):
        ...
        if self.use_pos_enc:
            pos_encoding = self.position_encoding(features,torch.zeros(features.size(0),18,18, dtype=torch.bool).to(device))
        ...
        diag_mask = torch.eye(output.size(1)).unsqueeze(0).repeat(output.size(0),1,1).to(device)
